...
def draw_hud(frame, results, total_scores):
    ...
    for face, truthfulness_score in results:
        ...
        if truthfulness_score >= 0.71:
            bar_color = (0, 255, 0)  # Green (Truthful)
        elif truthfulness_score >= 0.41:
            bar_color = (0, 255, 255)  # Yellow (Uncertain)
        else:
            bar_color = (0, 0, 255)  # Red (Deception)
        ...
        overlay_w, overlay_h = w, 110  # Adjusted height for proper text spacing
        ...
        cv2.putText(frame, f"Total Score : {avg_truthfulness:.2f}",
                    (text_x, text_y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        ...
        cv2.putText(frame, f"Conclusion : {conclusion}",
                    (text_x, text_y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, conclusion_color, 2)

        # Display developer credit
        cv2.putText(frame, "By Opposite6890",
                    (text_x, text_y + 75), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
